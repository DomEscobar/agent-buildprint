...
A fresh Codex run bootstrapped the live Buildprint package with `agb start` and built a clean-room neutral Node.js proof from `.buildprint/snapshots` only. The proof models the inspired/adaptive methodology harness, not an official Superpowers plugin rebuild and not a production runtime adapter.
