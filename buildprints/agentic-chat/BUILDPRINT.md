...
2. `agentic_chat` — a production-grade model-driven action loop: **stateful harness** + **ReAct tool loop** where the model (via provider tool/function calling, not slash commands or keyword pattern-matching) decides when to run an allowlisted tool, skill, MCP server, or memory action. Runs pass **context packing with trust zones**, **prompt-injection action screening**, **scoped capability grants**, **runtime budget policy**, **loop detection**, **tool error recovery**, **HITL approval**, **verifier/done-check**, **run receipts**, and **append-only session events**. Every side effect passes an inline approval gate, persists an audit record, and feeds its observation back into the loop for the next step, with bounded retry and a resumable trace. Built and proven through `loops/04-agentic-loop-runtime.md`. Technique basis: `references/runtime-techniques-basis.md`.
3. `agentic_swarm` — the full claim: a supervisor decomposes a goal into typed subtasks, dispatches **real parallel subagents** with isolated context and scoped tool access, then synthesizes their fan-in results into one goal-tied answer with honest partial-failure handling, cancellation, and resumable runs. Built and proven through `loops/05-swarm-dispatching.md`.
