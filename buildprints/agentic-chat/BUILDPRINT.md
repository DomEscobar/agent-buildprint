...
2. `agentic_chat` — a model-driven action loop where the **model itself** (via provider tool/function calling, not user-typed slash commands or keyword pattern-matching) decides when to run an allowlisted tool, skill, MCP server, or memory action. Every side effect passes an inline approval gate, persists an audit record, and feeds its observation back into the loop for the next step, with bounded retry and a resumable trace.
3. `agentic_swarm` — the full claim: a supervisor decomposes a goal into typed subtasks, dispatches **real parallel subagents** with isolated context and scoped tool access, then synthesizes their fan-in results into one goal-tied answer with honest partial-failure handling, cancellation, and resumable runs.
