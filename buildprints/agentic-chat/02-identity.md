...
This file runs after `01-setup.md` and before `loops/*`. Its job is not to hand the builder a pre-baked style sheet. Its job is to force high-reasoning UI identity work after the local harness and architecture baseline exist, but before scaffolding and phase code turn a weak product shape into permanent implementation.
...
- If the local frontend skill is missing, return to `01-setup.md` and initialize the project-local harness. Do not continue by improvising from memory.
...
- Agentic action must live inside the chat loop. At the foundation level the inline affordances are provider/route state, usage/trace disclosure, and retry/recovery buttons attached to the message or composer moment where they matter. As the product climbs the capability ladder (`capability_maturity` in `blueprint.yaml`), the inline affordances grow: model-proposed action chips, inline tool/MCP approval/block cards, memory save/recall prompts, restore points, action results, delegation records, and a compact live swarm panel (parallel subagents with per-worker status, each expandable to its own trace). Design the inline, chat-native pattern for each, and ship a level's affordance only once that level is genuinely built and proven — never an empty or faked version. The swarm panel and approval cards must stay subordinate to the conversation thread; they enhance chat, they do not become a task dashboard, kanban, or run-control console.
...
Then think deeply about the product, user, artifact type, golden path, central output, risk, density, review proof, and what a confused first-time user would misunderstand. Use the user's **design direction / visual wishes** answer from `00-goal.md` (aesthetic, brand/tone, reference products to emulate or avoid, density, light/dark intent) as the binding seed for this work; do not override it with a generic default. Generate a full local UI identity plan in the implementation project. Do not re-ask for visual direction that the design-direction hard-stop already captured; only ask when that answer is missing or a hard-stop says the product identity itself is unknown.
...
- The local frontend skill was loaded, or a missing-harness blocker routed back to `01-setup.md`.
