...
- `docs/architecture.md` - selected stack, runtime topology, adapters, persistence, deployment posture, state ownership, golden path, central output contract, proof strategy, selected typed quality gates, command/proof paths, blockers, and claim ceilings. It must include a `## User Intent Contract` section copied from the confirmed question answer, mapping the representative goal and acceptance criteria to runtime components, policy boundaries, a live proof scenario, and explicit non-goals.
...
- Do not start loop work while `.buildprint/decisions.md` still contains the empty stub; all hard-stop questions from `00-goal.md` (provider/model, stack/framework, design direction, and the safety/scope gates) must be resolved first with `answer`, `confirmed_by`, `reversible`, and `blocks_setup` fields.
...
- Do not skip `agb harness init . --provider agents` or the equivalent manual local harness creation before identity or loop work.
- Do not skip `agb harness check . --provider agents` after harness initialization or `agb harness checkup . --provider agents` before loop implementation; record checkup warnings as setup blockers or accepted claim ceilings.
...
- `.buildprint/decisions.md` records confirmed answers (or honest blockers) for all hard-stop questions from `00-goal.md` (including provider/model, stack/framework, and design direction alongside the safety/scope gates); each row includes `answer`, `confirmed_by`, `reversible`, and `blocks_setup`; `confirmed_by` is `user`, `explicit_user_delegation`, or `blocker`, never `agent_assumption`; the file must not contain the empty "No implementation decisions recorded yet" stub;
...
- `agb harness check . --provider agents` passes, and `agb harness checkup . --provider agents` has no unresolved fail result before loop work;
...
- `02-identity.md` can start without guessing the architecture or whether the local skill harness exists.
