...
0. Foundation scaffold verdict: confirm the implementation project has a real base project structure, root `AGENTS.md`, `architecture.md`, `engineering-standards.md`, `test-strategy.md`, and `ui-identity.md` when UI-bearing. `architecture.md` must include `Boundary map` and the other setup-gate sections before code can be counted as phase-ready.
