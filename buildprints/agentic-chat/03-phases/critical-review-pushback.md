...
- Failure mode: self-review, buffered fake streaming, missing screenshot evidence, generic dashboard/workbench UI, thin architecture, lazy setup diagrams, generic project structure, dead controls, Track B/C failure, or unresolved blockers prevents PASS.
...
- `architecture/*.md`, `PROJECT_STRUCTURE.md`, and `ARCHITECTURE_STRUCTURE_TRACE.md` when present.
...
- `.buildprint/claim-gates.json` and `.buildprint/claim-check.md` before scoring any `agentic_chat` or `agentic_swarm` claim.
...
5. Before scoring, inspect the setup architecture/structure packet. `architecture/*.md` must contain product-specific Mermaid diagrams with labeled edges and implementation mappings. `PROJECT_STRUCTURE.md` must organize files by Agentic Chat runtime/product responsibility with ownership and exclusions. `ARCHITECTURE_STRUCTURE_TRACE.md` must map components to files to proof and record score `4` or `5`, or a blocker with a lowered claim ceiling. If this packet is missing, generic, stale, unlabeled, unmapped, or scored below `4`, trigger the setup architecture auto-fail.
6. Before scoring any higher maturity claim, inspect `.buildprint/claim-gates.json`. If it is missing, invalid, cites non-existent evidence, or computes a lower `highest_honest_claim` than the UI/README/HANDOVER claims, fail runtime/proof integrity and lower the completion claim. Do not override phase 06 with reviewer confidence.
7. Before scoring, list the **five worst flaws** a demanding human would call out. Each flaw must cite concrete evidence: a screenshot region, `file:line`, or command output. Do not score until this section exists.
8. List the artifact surfaces or commands reviewed, including screenshots, browser paths, API/CLI checks, exports, persistence/readback checks, architecture/structure files, and blockers. Capture UI screenshots using the screenshot-capture protocol: the named tool chain (Playwright MCP, then the IDE/Cursor browser screenshot tool, then a project script, then ask the user), at viewports 375, 768, 1280, and 1440, across the default plus the real empty/loading/error/blocked/success states, saved to `.buildprint/screenshots/` with viewport-and-state names. Do not run the screenshot reviews below on a desktop-only or single capture. The reviewer must explicitly compare the first-screen screenshots against `docs/ui-identity.md`, `docs/DESIGN.md`, the forbidden silhouette, and the named adjacent at-risk silhouette; if the screenshot is indistinguishable after ignoring copy/color/icon changes, or if it violates the promised visual taste system, fail Track B.
9. Run screenshot delta review when this is a rerun or redesign: compare old and new screenshots and state whether the dominant surface, interaction model, creative/operator object, user flow, and information hierarchy changed. Palette, copy, labels, spacing, iconography, and section-title changes alone fail this review.
10. Run a progressive-disclosure screenshot review for every UI-bearing result, even when there is no prior screenshot. Count the permanent first-screen surfaces and name which capabilities are visible now versus reachable later. The review fails if the screenshot permanently displays more than one dominant creative/operator surface, one supporting context surface, and one action/status surface, or if major product capabilities compete on one page.
11. Run **objective auto-fail checks** (see below). Record each check as pass or triggered. Any triggered check caps affected categories and forces a repair loop.
12. Score the artifact on the rubric below from 0 to 5 per category. Each score must cite a concrete artifact — screenshot region, `file:line`, or command output. A prose-only justification without a cited artifact is invalid for that category and counts as score 0 for that category.
13. Compute the total score out of 60.
14. Pass threshold: at least 50/60 overall, no category below 4, no unresolved high-severity finding, no failed experience-originality, screenshot delta, or progressive-disclosure review, no triggered objective auto-fail, and a valid independent reviewer. **Track B (product/UI) and Track C (decisions/honesty) must both be fully clear before any PASS or PENDING_RECHECK verdict. A build may not reach PASS or PENDING_RECHECK by resolving only Track A (runtime/proof) while Track B or Track C failures remain open.**
15. If the score fails, run a repair loop:
...
16. Cap self-contained repair loops at five iterations unless the user explicitly asks for more. Stop early only for a genuine blocker, not because the review is uncomfortable.
17. Update `.buildprint/progress.md`, `.buildprint/blockers.md`, and handoff notes with the final score, repaired flaws, proof commands, and remaining risks.
...
- **Lazy setup architecture or structure**: `architecture/*.md`, `PROJECT_STRUCTURE.md`, or `ARCHITECTURE_STRUCTURE_TRACE.md` is missing after setup, diagrams use generic boxes or unlabeled edges, components do not map to planned files/modules, the project tree is dominated by vague folders such as `components`, `utils`, `services`, `api`, `pages`, `lib`, `helpers`, or `misc` without responsibility/exclusion/proof mapping, or the recorded architecture score is below `4` → cap **Runtime/proof integrity**, **Handoff quality**, and **Ad hoc flaw repair** at 2.
...
- **Chat-native genre drift**: for Agentic Chat, the first viewport is not mostly conversation thread plus composer/input, or agentic actions are not embedded as inline chat/composer affordances. A mission sheet, guided-run launcher, task dashboard, status lane, or form-first workflow that pushes the transcript below the fold fails even if it has a clearer action loop → cap **Product intent fit**, **UX clarity**, **Experience originality**, and **Visual/design execution** at 2.
- **Consumer chat craft failure**: for Agentic Chat, the default first viewport looks like a harness demo rather than a polished chat product: seeded approval/memory/restore cards before user intent, giant blank dead zone, internal route/provider/memory labels as primary UI, boxy uniform card treatment, cramped mobile chips/composer, or generic empty-state copy → cap **UX clarity**, **Visual/design execution**, **Experience originality**, and **Responsive/accessibility quality** at 2.
...
Any category capped at 2 triggers the mandatory repair loop in step 14.
...
- Setup architecture proof is included in the review: `architecture/*.md`, `PROJECT_STRUCTURE.md`, and `ARCHITECTURE_STRUCTURE_TRACE.md` are listed as reviewed surfaces with pass/fail/blocker, and any architecture drift is recorded.
