...
For UI-bearing phases, `phase_core_passed` requires at least one user action path through a UI/controller boundary into runtime behavior and back into a visible state or readback result. Static state cards, dead buttons, generic dashboard panels, or review prose do not satisfy the UI action path. A full-suite browser workbench also cannot pass the UX gate with a single embedded HTML/CSS/JS file, default browser controls, stacked forms, raw text lists in place of domain visualization, or a screenshot that reads as a quick local MVP.
...
- `## Domain interaction model`: graph/simulation/report/history affordances that match the product domain instead of raw text lists or disconnected forms.
...
- Browser automation that only proves elements exist does not upgrade `visual_quality_gate`. To upgrade it, the UX review must inspect screenshot evidence and reject default-control shells, generic dashboards, stacked form pages, raw text-list substitutes for graph/simulation/report workbenches, unreadable density, weak hierarchy, missing focus/disabled states, or mobile/desktop overlap.
