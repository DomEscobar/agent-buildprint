# Loop 03 - Media Policy And Wavespeed
...
Build the public/private media policy gate and Wavespeed image skill. Live Wavespeed calls must sit behind a provider adapter with typed blocked/error states; tests must use mock mode. The operator path should turn a grounded draft or persona prompt into a media job, block unsafe/private media with structured reasons, and persist the media job status for retry or audit.
