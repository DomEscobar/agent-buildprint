# Loop 04 - Social Planner And Publisher
...
Create the social planner, social autonomy rules, draft store, media-job linkage, QA notes, approval state, mock publisher, and secured browser/noVNC publishing handoff. Drafts must include `groundedIn` references and remain unapproved until QA. Publishing must be manual or mock by default; any live social/browser path must require explicit approval, local-only auth, and audit evidence.
