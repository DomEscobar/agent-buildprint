# Loop 02 - Persona Memory Runtime
...
Implement persona canon, boundaries, analyzer adapter, runtime context builder, storage adapter, and the strict split between user relationship memory and persona self-state. The real product path should process a message through a mockable LLM JSON analyzer, update only the correct memory store, preserve private state boundaries, and show the operator what was remembered, what was not remembered, and why.
