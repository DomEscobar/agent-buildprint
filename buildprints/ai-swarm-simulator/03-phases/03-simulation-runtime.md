...
Connect the social simulation runtime seam to the graph workbench and preserve the source entity-to-agent and OASIS action-log semantics. The user should be able to configure or review agents/roles derived from graph context, including source entity id/type, persona, stance or sentiment bias, influence weight, activity level, active hours, and platform settings. The user should then start a simulation run when prerequisites exist, watch status/trace updates, inspect simulated posts/replies/reactions/searches/follows/reposts/quotes/comments, and receive an honest blocker when provider credentials, graph context, OASIS runtime, or local runtime support are missing. Preserve the MiroFish/OASIS-style Twitter/Reddit source signal without pretending the full original runtime is available if it is not. The simulation state model should include run id, config, participant/agent profiles, platform mode, provider status, graph snapshot reference, per-platform running/completed state, current round/hour, feed events, action logs/traces, result status, and blockers. Each action should retain platform, round, timestamp, agent identity, action type, action args, result, and success so report generation can prove where claims came from. UI controls must not be dead: prepare environment, start, stop/cancel where supported, inspect trace, inspect feed item, inspect agent, and retry should either work or explain exactly why they cannot. Provider test/status should be reusable by this phase and should never log secrets. The workbench should make clear whether a run is simulated locally, provider-backed, deterministic, blocked, failed, or complete.
...
Run unit/API/browser smoke checks for environment preparation, simulation status transitions, action-log creation, and platform timeline rendering. Verify missing provider, graph, OASIS, or runtime produces a blocked state. Inspect that trace/feed/result UI uses runtime state, action logs, agent profiles, and graph references rather than canned success text.
