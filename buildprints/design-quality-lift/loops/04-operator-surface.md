...
Apply the direction's voice to every piece of microcopy in the host. Make the host accessible at WCAG 2.2 AA minimum. Record information architecture decisions. this loop is where the direction becomes audible and the host becomes usable for everyone.

## Why this loop exists

LLM-default microcopy is generic friendly. "Oops!", "Whoopsie!", "Whoops, something went wrong!" — these are not voice. Accessibility is often treated as a checklist. IA is often assumed, not decided. this loop forces opinionated voice, real accessibility, and explicit IA decisions.
