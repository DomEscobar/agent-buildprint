# BUILDPRINT: Portable Local RAG Chat Workbench
...
- Product: Portable Local RAG Chat Workbench
- Primary outcome: Compact executable packet for a local-first RAG chat app with Ollama-compatible provider seams, durable SQLite/libSQL state, document ingestion, citations, memory/settings, and repeatable browser proof.
- Primary users: operators or developers implementing the mapped product workflow.
- Main surfaces: Provider Chat Runtime; Document RAG Indexing; Memory Settings Lifecycle; BUILDPRINT.md canonical start
