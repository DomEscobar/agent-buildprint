...
For Media4Agents, check [URL sprites](media4agents-url-sprites.md): one workflow and valid flags/size, public-token placeholder in public packet examples, identical URL reuse without cache-busting, and approval before first-load/prefetch/QA spend. Exercise visible loading/error states and pixelated display scaling only after approval; inspect actual sheet layout before slicing and record runtime animation. Offline checks do not establish provider access, CORS, billing or art quality.
