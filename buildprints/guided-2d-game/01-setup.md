...
Runtime implementation and proof require this foundation. Art direction, rights-cleared asset work and approved provider operations may start during setup once their own scope, rights, access and cost decisions are resolved; neither a completed gameplay loop nor a graybox is required. Mark in-engine approval pending until the real renderer is available, without delaying asset iteration. Configure engine simulation separately from rendering; a fixed step or delta-time contract must survive low/high FPS and background-tab return. Keep source assets private when needed, ship only rights-cleared static derivatives. Use `validation/README.md` to wire the shared validator into the real loader; fixtures are testing material, not game content. No backend is needed for local solo gameplay.
