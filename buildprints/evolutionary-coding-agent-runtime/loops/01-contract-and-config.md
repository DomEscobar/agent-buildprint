...
Define **forbidden mutation** / **forbidden-file guardrails** before candidate execution: secrets, credentials, deployment configs, billing surfaces, and production data paths are never editable. Do not use blocklist-only mutation scope without documenting a user-approved exception.
