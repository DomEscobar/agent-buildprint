...
## Setup authority additions

Architecture principles: preserve the clean-room novel-to-storyboard workbench scope, keep provider adapters, persistence, browser proof, and evidence-ledger claim promotion explicit, and do not downgrade to a static UI shell, toy local demo, or copied source implementation.

The implementation-project `AGENTS.md` plan belongs in the generated implementation project after setup. It must point future agents to `.buildprint/next-agent.md`, the active phase, required role contracts, runtime evidence rules, and the clean-room boundary. Do not ship project `AGENTS.md` inside this Buildprint packet.

Execution authority model: `03-phases/phase-flow.md` is the execution authority for phase work. Each active phase must use bounded handoff artifacts for every required role, collect matching returns/reviews/proof, and only then append runtime evidence.

Phase start gate: before phase implementation, complete `01-questions.md`, complete this setup file, read `03-phases/phase-flow.md`, resolve every active `requires_roles` entry to `06-contracts/<role>.md`, and create the phase-run artifacts under `.buildprint/phase-runs/<phase-id>/`.
