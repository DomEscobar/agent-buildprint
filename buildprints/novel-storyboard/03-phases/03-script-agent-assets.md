...
- Mode lens: this phase proves the product outcome flow for turning narrative events into structured script artifacts and deduplicated production assets.
- Shared proof spine: preconditions are persisted events; entrypoints are browser/API script generation and asset extraction actions; execution behavior runs outline, adaptation, writing, supervision, and asset extraction stages through provider/tool boundaries; state effects include script beats/episodes, roles, scenes, props, job status, and editable workspace records; observable proof includes structured output tests, asset dedupe proof, UI readback, and provider-blocked states; failure/recovery covers invalid event state, provider/runtime failure, duplicate assets, retry/cancel, and restart readback.
...
- Proof id: proof-03-script-agent-assets
...
Required runtime evidence row must use `phase_id: 03-script-agent-assets` and write to `.buildprint/evidence/evidence-ledger.jsonl`.
