# Phase 02 — Novel Event Ingestion
...
- Mode lens: this phase proves the product outcome flow for importing ordered novel/chapter text and transforming it into structured narrative events.
- Shared proof spine: preconditions are an authenticated persisted project; entrypoints are browser/API ingestion and event-extraction actions; execution behavior validates ordered text, normalizes input, invokes deterministic or configured provider adapters, and stores events; state effects include chapters, extraction jobs, event records, provider mode, and error reasons; observable proof includes ingestion readback, extraction schema tests, job/runtime trace, and browser state proof; failure/recovery covers empty text, malformed chapters, provider errors, retry, cancel where applicable, and restart readback.
...
Required runtime evidence row must use `phase_id: 02-novel-event-ingestion` and write to `.buildprint/evidence/evidence-ledger.jsonl`.
