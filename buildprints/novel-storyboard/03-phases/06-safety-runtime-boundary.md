# Phase 06 — Safety Runtime Boundary
...
- Mode lens: this phase proves the product outcome flow for operating sensitive runtime/admin/provider/export surfaces without overclaiming safety or live readiness.
- Shared proof spine: preconditions are the full local pipeline surfaces; entrypoints are provider settings, upload/file handling, admin import/export/clear/delete, evidence review, and readiness checks; execution behavior enforces auth/session, secret redaction, destructive confirmation, audit, upload limits, provider sandbox/live split, and no-fake claim boundaries; state effects include audit rows, admin lifecycle records, exported backups/manifests, safety blockers, and evidence rows; observable proof includes denied-path, invalid-input, redaction, destructive-confirmation, readiness, no-fake, and deployment-blocker checks; failure/recovery covers unauthorized admin access, secret leakage attempts, path traversal, failed import/export, missing live credentials, rollback/readback, and public deployment blockers.

## Mapped product obligations

Source surface IDs: SRC-ADMIN, SRC-PROVIDERS, SRC-RUNTIME, SRC-UPLOADS

Product obligations: OBL-SAFETY.

Mapped product obligation refs:
- Vendor config routes and VM helper enable provider adapter customization.
- Upload/file helper writes media under local data roots.
- Database admin routes export/import/clear/delete data.
- Electron/Docker surfaces exist in Toonflow but are outside the selected clean-room binding slice.
...
Stable obligations:
- secrets are configured by name and never printed
- uploads are contained and MIME/type checked
- destructive actions are confirmation/audit gated
- live providers are optional and blocked from default proof

Free choices:
- exact secret storage backend
- confirmation UI design
- deployment target if later approved
...
- Proof id: proof-06-safety-runtime-boundary
...
Required runtime evidence row must use `phase_id: 06-safety-runtime-boundary` and write to `.buildprint/evidence/evidence-ledger.jsonl`.
