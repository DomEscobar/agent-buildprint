...
- Mode lens: this phase proves the first product outcome flow: a user enters a private project workspace, configures project/provider settings, and receives durable readback plus denied-path behavior.
- Shared proof spine: preconditions are a scaffolded app and selected auth/session default; entrypoints are browser/API project workspace actions; execution behavior validates identity, project metadata, and provider settings; state effects include persisted user/session/project/settings records; observable proof includes browser/API traces, denied-path tests, and persistence readback; failure/recovery covers invalid login, invalid project payload, missing provider config, unauthorized access, and restart readback.
