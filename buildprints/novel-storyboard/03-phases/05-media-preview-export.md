...
- Mode lens: this phase proves the product outcome flow for creating media preview tasks and exporting a reviewable manifest/artifact set.
- Shared proof spine: preconditions are storyboard panels with asset refs; entrypoints are browser/API media preview and export actions; execution behavior creates image/video preview jobs through provider adapters, exposes status/progress/failure, and writes export manifests; state effects include media task rows, generated file refs, export manifests, provider mode, and limitations; observable proof includes task lifecycle tests, file containment/readback, manifest validation, and browser preview/export proof; failure/recovery covers missing provider config, task failure, cancel/retry, oversized artifacts, export errors, and restart readback.
...
- Proof id: proof-05-media-preview-export
...
Required runtime evidence row must use `phase_id: 05-media-preview-export` and write to `.buildprint/evidence/evidence-ledger.jsonl`.
