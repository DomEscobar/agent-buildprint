...
0. For Phase 01, verify the Foundation scaffold gate from `02-project-setup.md`: the implementation project has a real base project structure, root `AGENTS.md`, `.buildprint/setup.md`, `architecture.md`, `engineering-standards.md`, `test-strategy.md`, and `ui-identity.md`. The root `AGENTS.md` must list those files as mandatory reads. If the scaffold, base project structure, or standards files are missing, stop and satisfy the setup gate before coding.
...
## Foundation scaffold rule

The Foundation scaffold gate is a hard precondition for Phase 01. It must create the selected stack's base project structure plus `architecture.md`, `engineering-standards.md`, `test-strategy.md`, and `ui-identity.md`. `architecture.md` owns the Boundary map and downstream extension map. `engineering-standards.md` owns clean code, validation, persistence, provider, worker/runtime, UI, and test standards. `test-strategy.md` must define deterministic blocker/e2e/runtime timeout behavior, including exiting deterministically when browser/runtime proof cannot run. Later phases must read the scaffold files through root `AGENTS.md` before code edits.

## Evidence append prohibition

You may not append runtime evidence to `.buildprint/evidence/evidence-ledger.jsonl` until the required phase-run plan, team gates, handoffs, returns, reviews, verification proof, and blocker/proof notes exist for the active phase.
