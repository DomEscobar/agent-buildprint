...
Every `.buildprint/phase-runs/PHASE_ID/` directory name and every evidence row `phase_id` must exactly match a `phase_id` listed in `.buildprint/snapshots/03-phases/phase-index.yaml`. Creating phase-run directories with names that do not appear in the index, or writing evidence rows with invented `phase_id` values, is a fake-completion violation.
...
Create `.buildprint/phase-runs/PHASE_ID/phase-preflight.yaml` before implementation. Required fields:
...
- `.buildprint/phase-runs/PHASE_ID/phase-preflight.yaml`
- `.buildprint/phase-runs/<phase-id>/plan.md`

Before evidence:

- `.buildprint/phase-runs/<phase-id>/proof.md`
- `.buildprint/phase-runs/PHASE_ID/evidence.json` with command/artifact manifest
