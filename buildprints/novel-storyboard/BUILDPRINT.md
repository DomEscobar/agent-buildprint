...
- Primary outcome: Source-independent execution packet for a Toonflow-inspired creative production workbench: project workspace, ordered novel ingestion, event extraction, script and asset generation, storyboard sequencing, media task preview/export, and safety/runtime gates.
- Primary users: operators or developers implementing the mapped product workflow.
- Main surfaces: browser workbench, API service, worker/runtime jobs, provider adapters, durable persistence, generated artifacts, media/export manifests, and evidence ledger.
- What this packet must not become: a generic local MVP, static demo, source clone, route parity exercise, dead-control UI, or single-file product shell.
...
Blueprint mode is classified before implementation as `product` with `outcome_flow` phases. The packet remains `PROOF_REQUIRED`: the implementation agent must build and prove the product from this packet without reopening the original source repository.
