...
## Setup And Standards Check

Before accepting phase evidence, verify the Foundation scaffold gate: root `AGENTS.md` exists, `architecture.md` is read and names the relevant boundaries, `engineering-standards.md` is read and includes clean code rules plus validation/persistence/provider/worker/runtime standards, and `test-strategy.md` is read and includes Test standards with deterministic blocker/e2e/runtime exit behavior. The Setup/standards verdict must say whether missing scaffold content blocks evidence.
