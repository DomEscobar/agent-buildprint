...
Create the implementation foundation and the first visible story-making loop for AI Story Maker. The first loop should let a writer start a story package from a premise and optional seed notes, validate the input, create durable local package state, and see an initial studio surface with story package status, cast/world placeholders that are honestly empty or fixture-labeled, and a clear next action to build the outline. This is not a marketing page or a chat demo. The first screen after setup should already feel like the editorial story-room workbench described in `02-uiux-decision.md`.
...
- Do not build a generic landing page when the mapped product is a working story studio.
