...
Use this loop for the active loop only. Do not read every loop upfront. Do not turn phase execution into paperwork.

## How to run a phase

1. Read `BUILDPRINT.md`, `00-goal.md`, `01-setup.md`, and `02-identity.md` before every active loop. For UI-bearing artifacts, `02-identity.md`, the generated local UI identity, and `docs/DESIGN.md` are standing contracts, not optional polish files: `docs/ui-identity.md` governs product/interaction fit, and `docs/DESIGN.md` governs visual taste and craft.
...
4. Before editing code, state a short thinking checkpoint in the working response or task plan: active loop, smallest real vertical user/operator path, 3-7 likely failure modes, proof plan, and claim ceiling. This is not a deliverable file. Do not create phase-run paperwork by default.
...
Final completion is impossible until phase `review.md` has run and either passed or recorded an external blocker. Treat it as the last phase in the phase graph, not as optional review prose.
...
- If setup is missing architecture, local skill harness, commands, env, proof surfaces, or setup receipt, return to `01-setup.md`.
