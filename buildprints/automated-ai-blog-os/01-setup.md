...
Then create or update the implementation project foundation. Do not start `02-identity.md` or `loops/*` until setup has enough concrete decisions to guide coding and the project-local skill harness exists.

## Setup objective

Create the real base project structure for Automated AI Blog OS, centered on a source-grounded editorial pipeline that turns captured sources into useful draft posts and blocks publishing until claims, SEO, feeds, build, and approval gates pass. Choose a stack that can actually implement the product contract and golden path mirrored in `blueprint.yaml`. Define the module boundaries, persistence model, provider/runtime seams, commands, safety rules, and verification strategy. The goal is not to over-plan; the goal is to prevent the next agent from building generic slop.
...
- Do not start loop work while `.buildprint/decisions.md` still contains the empty stub; all five hard-stop questions from `00-goal.md` must be resolved first.
...
- `.buildprint/decisions.md` records confirmed answers (or honest blockers) for all five hard-stop questions from `00-goal.md`; the file must not contain the empty "No implementation decisions recorded yet" stub;
