...
- `selected-buildprint/`: full-suite Buildprint spine plus 15 capability packs.
...
- `find source-real/src/routes` and `rg` inspections of env/auth/routes/db/provider surfaces.
...
- Capability pack count check: 16 capability directories, each with CAPABILITY/VERIFICATION/IMPLEMENTATION/CONTRACTS files.
...
- `node --check source-real/data/serve/app.js`: passed.
...
- `test -d source-real/node_modules`: node_modules missing, so dependency-backed TypeScript/build/runtime checks were not run.
