...
Execution mode defaults to `continuous-full-suite`: load only the active capability pack, prove it, update `CURRENT_STATE.md`, then continue to the next dependency-ready pack in `CAPABILITY_INDEX.md`. Use `active-capability-handoff` only when explicitly selected. Do not read unrelated packs upfront in either mode.
...
7. After proof passes, update state and ledgers. In `continuous-full-suite`, advance to the next dependency-ready pack and continue in the same session. In `active-capability-handoff`, stop after naming the next pack and preserving handoff state.
