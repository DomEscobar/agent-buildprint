...
Proof id: proof-contracts-adapters
...
Required runtime evidence row must use `phase_id: contracts-adapters` and write to `.buildprint/evidence/evidence-ledger.jsonl` after phase-flow artifacts exist. The packaged `05-evidence/evidence-ledger.jsonl` is seed evidence only.
