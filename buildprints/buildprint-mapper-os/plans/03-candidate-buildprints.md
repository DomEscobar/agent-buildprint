...
- Mark incomplete or risky capabilities `INCLUDED_NEEDS_PROOF`, `INCLUDED_BLOCKED`, or `INCLUDED_RISKY_REQUIRES_HARDENING`; use `OUT_OF_SCOPE_BY_USER_ONLY` only for explicit user exclusions or target boundaries.
- For full-suite intent, preserve every blocker and require hierarchical output.

## Exit Criteria

- Candidate list has readiness classifications for ready, needs-proof, blocked, risky-hardening, user-excluded, and test-only capabilities.
