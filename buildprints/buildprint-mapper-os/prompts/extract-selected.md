...
- `BUILDPRINT.md`: heading `# BUILDPRINT: <mapped-app>`, section headings `## Product brief`, `## Final product at a glance`, `## Required read order`, `## Project setup gate`, `## Implementation loop`, `## Completion semantics`, `## Repair routing`, `## Phase discipline`, and `## Handover`. `## Completion semantics` must state that bounded proof is not product completion, that review prose/status notes are not evidence, and that every pass verdict requires rerunnable command output or an existing artifact path (copy from `templates/executable-packet/BUILDPRINT.md`; do not omit).
...
Handover rule: `## Handover` must instruct the agent to write `.buildprint/handover.md` after all phases complete (or when the run stops with honest blockers). Copy the six-section structure from `templates/executable-packet/BUILDPRINT.md`: Build state, North star comparison (one row per glance surface), What was built, What is missing or blocked, What the human must do next, Micro roadmap. The handover compares real built artifacts and evidence against `## Final product at a glance`; a surface counts as built only when its owning phase reached `phase_core_passed` and done-looks-like criteria are satisfied by real proof.
...
`02-project-setup.md` must define architecture boundaries (browser workbench, API service, worker/runtime boundary, provider adapters, repositories, persistence, e2e harness) AND, in `## Architecture decisions`, name a concrete production-grade target stack. These are two different rules and must not be conflated:

- Do not clone the **source repository's specific** frameworks by default. The source stack is a hint, not a mandate; record that its internals are replaceable.
- Do choose and **name a real, mainstream, actively-maintained target stack** for the implementation: a concrete framework/runtime, package/build system, and — for UI-bearing products — a component/UI framework and a styling system (utility-first such as Tailwind, or a design-token/component library). "Stack-agnostic" means free to pick the best fit, not "leave the stack unnamed" and not "default to vanilla single-file HTML/CSS/JS or hand-rolled equivalents."

Keep the product north star (`## Product brief`, `## Final product at a glance`) in capability language with no stack names; the concrete stack lives only in `## Architecture decisions`. A `02-project-setup.md` that leaves Framework/runtime, Package/build, or (for UI) Styling/UI system unnamed, or that allows a single-file shell, is invalid.
...
`## Foundation scaffold gate` in `02-project-setup.md` must include the **Runnable verification gate** subsection from `templates/executable-packet/02-project-setup.md`: require `verify:no-fake`, `verify:phase-artifacts`, and `verify:stack-baseline` scripts (or documented stack equivalents), mandate running them before runtime evidence, and require pasted stdout in `.buildprint/phase-runs/<phase-id>/proof.md`. Do not reduce this to prose-only "run tests". `verify:stack-baseline` must encode the concrete stack from `## Architecture decisions` (build system/manifest present; for UI-bearing products a real component framework and styling system are declared dependencies; no single-file HTML/CSS/JS shell) and must pass before Phase 01.
...
Write interaction depth as behaviors, not as a labeled control inventory. An interaction such as zoom, pan, drag, or selection is a gesture/handler plus an observable state transition, not a button or pill that names the capability. The phase must not invite the implementer to render its capability/interaction checklist as on-screen labels; doing so produces dead capability-label controls. Each named interaction must be proven by exercising the real control and asserting its effect, not by asserting a label element exists.

UI-bearing phases must forbid scaffolding leak in the product surface: the implemented UI must not display Buildprint/phase identifiers ("Phase 01", "BUILDPRINT…"), raw internal ids (`project_…`, `graph_…`), or proof/test vocabulary ("deterministic mode", "local contract proof only", "test-only"). Provider-blocked and sandbox states must be written in product language. State this in the phase `## UX/UI requirements` and in the `visual_quality_gate` / screenshot critique.
