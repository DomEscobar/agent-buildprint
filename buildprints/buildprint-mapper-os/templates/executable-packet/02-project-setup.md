...
### Craftsmanship alignment in active context

Before Phase 01 starts, root `AGENTS.md` must put the generated alignment speech in active context for every future coding agent. Do not create a separate harness document. The alignment belongs where agents already look before editing.

The root `AGENTS.md` must tell implementers, in plain language, to act like senior product engineers and designers: treat the Buildprint as the minimum contract, choose the best tasteful interpretation when specs are thin, deepen the core feature rather than widening scope, add obvious local interactions/states/validation/persistence/copy polish, and reject technically passing work that feels cheap.

Allowed enhancement: local, safe improvements that make the mapped product more credible without changing the product category, adding external paid dependencies, inventing unrelated features, weakening safety, or hiding missing live-provider behavior. Disallowed enhancement: new product direction, unrelated surfaces, fake AI, ornamental UI, or broad rewrites that dodge phase obligations.
