...
Define and prove the document processing task loop: task objective, tool/action inventory, plan/execute/observe loop, completion check, human review-note points, recovery/escalation, and log proof.

## Phase mode contract

- blueprint_mode: automation
- phase_style: task_loop_contract
- Lens: this phase defines the task loop contract — objective, tool boundaries, plan/execute/observe loop, completion check, review-note points, and log proof.
- Shared proof spine:
  ...
  - Execution behavior: plan action, select tool, execute tool, observe result, reflect, repeat until completion check met.
  - State/artifact effects: task log log, output artifact, review-note checkpoint records.
  - Observable proof: log proves loop execution, tool calls, completion check trigger, and at least one review-note point.
  - Failure/recovery: tool failure, max_iterations reached, review-note rejected.
...
- completion check: all inputs processed; max_iterations reached; review-note rejected.
- review-note points: before writing final output when approval_required is true.
- Recovery/escalation: on tool failure, retry once; on max retries, escalate with log artifact.

## Behavior compatibility contract

- Disposition: preserve task loop capability; agent internals may differ.
- Equivalent target behavior: plan/execute/observe loop, completion check, review-note points, log proof.
...
2. Add completion check detection and max_iterations guard.
3. Add review-note point with checkpoint record.
4. Run log-based proof and stop-condition tests.
