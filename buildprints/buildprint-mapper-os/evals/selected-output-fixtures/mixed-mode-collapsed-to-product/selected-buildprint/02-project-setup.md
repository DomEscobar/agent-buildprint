...
- Blueprint mode:
  ...
  - Why this mode fits: fixture maps two distinct phase lenses: product outcome flow for record ingest, integration boundary transaction for payment provider.
- Product / capability: fixtures/executable-packet-mixed-good multi-mode packet
- Phase 01 uses blueprint_mode: product, phase_style: outcome_flow.
- Phase 02 uses blueprint_mode: integration, phase_style: boundary_transaction_contract.
...
- Framework/runtime:
  ...
  - Evidence: mapping, mixed mode requirements, and phase proof gates.
- Package manager:
...
- UI architecture: for mixed-mode: not UI-bearing unless explicitly added; define the callable/import/CLI/operator surface.
- Product composition: for mixed-mode, define the mode-specific composition, flow, or operation sequence.
...
Promote compiled mixed observations into implementation contracts before starting phases:

- Route/API/job prefixes and handlers: mapped note fixtures/mixed-target records the mixed capability.
- Request/response payloads and validation errors: mixed obligation requires validation and returns result.
...
|---|---|---|---|---|---|---|
| SRC-PRODUCT-INGEST | mapped note: fixtures/mixed-target/ingest:1-20 | Product ingest outcome flow: accept, validate, persist, readback. | preserve | Equivalent target behavior: product mode outcome flow with UI, validation, persistence. | `03-phases/01-product-ingest.md` | Browser e2e upload to project readback. |
| SRC-PAYMENT-INTEGRATION | mapped note: fixtures/mixed-target/payment:1-30 | Integration boundary transaction: config, webhook/callback, idempotency, sandbox/live split. | preserve | Equivalent target behavior: webhook dispatch, idempotency key, sandbox mode for tests. | `03-phases/02-payment-integration.md` | Fake-provider proof proves dispatch and idempotency. |
...
- Assumption: production implementation uses mixed-appropriate proof gates.
...
- `03-phases/01-product-ingest.md`
- `03-phases/02-payment-integration.md`
