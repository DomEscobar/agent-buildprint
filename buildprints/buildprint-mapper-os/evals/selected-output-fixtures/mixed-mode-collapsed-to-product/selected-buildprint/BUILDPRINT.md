# BUILDPRINT: fixtures/executable-packet-mixed-good
...
- Product: fixtures/executable-packet-mixed-good
- Primary outcome: prove two distinct phase lenses: product outcome flow for record ingest, integration boundary transaction for payment provider
- Primary users: developers, operators, or consumers of this capability
- Main surfaces: browser workbench, API service, provider adapter, payment integration boundary
- What this packet must not become: a packet where every phase uses the same mode under the mixed label to avoid classifying
