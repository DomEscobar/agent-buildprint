...
This micro replay is dependency-free Node.js only. Do not choose Python, install packages, start Docker, or use external services.

Keep the implementation minimal: no CLI, HTTP server, frontend, worker daemon, Docker setup, or package-install path unless the active phase explicitly requires it. The target is one provider seam, one durable readback path, one proof/test command, and honest evidence.
