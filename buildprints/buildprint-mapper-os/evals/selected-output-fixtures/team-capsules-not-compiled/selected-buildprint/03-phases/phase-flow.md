...
For UI-bearing phases, a screenshot that looks like a local MVP, default-control shell, generic stacked form, or raw text-list substitute is a phase blocker until repaired or explicitly routed as non-upgrading scope debt.
...
- `## Visual quality bar`: hierarchy, spacing, typography, color, density, and product fit against `02-project-setup.md`.
- `## Domain interaction model`: how the UI exposes the product's real domain objects instead of generic cards, stacked forms, or raw text lists.
...
- Browser automation that only proves elements exist does not upgrade `visual_quality_gate`. To upgrade it, the UX review must inspect screenshot evidence and reject default-control shells, generic dashboards, stacked form pages, raw text-list substitutes for the domain, unreadable density, weak hierarchy, missing focus/disabled states, or mobile/desktop overlap.
