...
- Blueprint mode:
  - Primary: automation
  - Secondary: []
  - Phase style: task_loop_contract
  - Why this mode fits: fixture maps an automation agent with a task loop, stop conditions, approval points, and trace proof.
- Product / capability: fixtures/executable-packet-automation-good task loop
- Task scope: process inputs through plan/execute/observe loop.
...
- Framework/runtime:
  ...
  - Evidence: mapping, automation mode requirements, and phase proof gates.
- Package manager:
...
- UI architecture: for automation-mode: not UI-bearing unless explicitly added; define the callable/import/CLI/operator surface.
- Product composition: for automation-mode, define the mode-specific composition, flow, or operation sequence.
...
Promote compiled automation observations into implementation contracts before starting phases:

- Route/API/job prefixes and handlers: mapped note fixtures/automation-target records the automation capability.
- Request/response payloads and validation errors: automation obligation requires validation and returns result.
...
|---|---|---|---|---|---|---|
| SRC-TASK-LOOP | mapped note: fixtures/automation-target:1-40 | Implement task loop: plan/execute/observe, stop condition, approval point, trace. | preserve | Equivalent target behavior: plan action, execute tool, observe result, reflect, repeat until stop condition met or approval required. | `03-phases/01-task-loop.md` | Trace proves loop execution, stop condition trigger, and approval point. |
...
- Assumption: production implementation uses automation-appropriate proof gates.
