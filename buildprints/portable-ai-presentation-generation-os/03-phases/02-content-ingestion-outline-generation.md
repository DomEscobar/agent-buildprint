# Phase 02 — Content Ingestion and Outline Generation

## Product outcome

Users can start a deck from prompt text, uploaded documents, or provided slide markdown and receive validated outlines with streaming/progress semantics.

## Source evidence

- `models/generate_presentation_request.py` defines content, slides_markdown, instructions, tone, verbosity, web_search, n_slides, language, template, files, export_as, and webhook flags.
- `api/v1/ppt/endpoints/files.py` exposes upload, decompose, and update file routes.
- `services/documents_loader.py`, `services/liteparse_service.py`, and document conversion services support file parsing/OCR/decomposition.
- `utils/llm_calls/generate_presentation_outlines.py` produces presentation outlines.
- `api/v1/ppt/endpoints/outlines.py` exposes outline streaming.
...
- Source surface: `SRC-INGEST-OUTLINE` from `02-project-setup.md`.
...
- Prompt and file upload ingestion with validation and size/type limits.
- Document decomposition/OCR/parser path with failure states.
- Outline generation using provider adapter or deterministic fake provider.
- Streaming/progress endpoint or trace for outline generation.
...
5. Append proof or blocker rows to runtime artifact `.buildprint/evidence/evidence-ledger.jsonl` using `phase_id: content-ingestion-outline-generation`.
...
- upload/decompose APIs, generation request model, outline streaming route, provider adapter boundary, parser service boundary.

## State/runtime touched

- uploaded file metadata, temporary file paths, decomposed chunks, outline drafts, async task status, parse failures.
...
- Proof id: proof-content-ingestion-outline-generation
...
Required runtime evidence row must use `phase_id: content-ingestion-outline-generation` and write to runtime artifact `.buildprint/evidence/evidence-ledger.jsonl`, not the packaged seed ledger.
