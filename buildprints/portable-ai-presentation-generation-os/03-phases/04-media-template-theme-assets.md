...
5. Append proof or blocker rows to runtime artifact `.buildprint/evidence/evidence-ledger.jsonl` using `phase_id: media-template-theme-assets`.
...
- Proof id: proof-media-template-theme-assets
...
Required runtime evidence row must use `phase_id: media-template-theme-assets` and write to runtime artifact `.buildprint/evidence/evidence-ledger.jsonl`, not the packaged seed ledger.
