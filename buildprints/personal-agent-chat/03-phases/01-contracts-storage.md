...
Create the implementation foundation for Personal Agent Chat: core domain contracts, config model, durable local stores for sessions/messages/checkpoints/memory/traces/telemetry, and a bootstrap service that can expose current runtime state without live providers or network.
