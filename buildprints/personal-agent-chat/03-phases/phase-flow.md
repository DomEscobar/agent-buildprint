...
0. Confirm the `Foundation scaffold gate` in `02-project-setup.md` has produced a real base project structure, mandatory root `AGENTS.md`, `architecture.md`, `engineering-standards.md`, `proof-strategy.md`, and `ui-identity.md` when UI-bearing. If the scaffold is missing, stop before phase code, create it, and ensure any blocked setup or e2e/runtime proof is exiting deterministically instead of hanging.
