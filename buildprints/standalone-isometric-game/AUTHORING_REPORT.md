...
## Subsequent instruction cleanup — 2026-09-14
The historical records below remain unchanged. That instruction cleanup treated routine builds, tests and contract-required acceptance as part of authorized implementation unless the user explicitly restricted them; paid calls, uploads, reusable-skill writes, publishing and deployment still needed their own authority. Asset technique preceded optional provider selection: the recorded WaveSpeed/Bria and Media4Agents gaps applied only when those routes were selected. Current release-facing instructions are in `README.md` and `references/cli-integration.md`.

## 0.1.0 package note
The historical records remain evidence about their named sessions. The `v0.1.0` source tag contains the compatible CLI and this packet, but npm `agent-buildprint@0.1.0` is not published; public `0.0.17` lacks v2. This does not retroactively change hosted manifests, existing snapshots or historical source availability. Current installation and operational limits are documented in `README.md` and `references/cli-integration.md`.
