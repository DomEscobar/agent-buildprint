...
1. Read the [agent guide](https://agent-buildprint.com/buildprints/standalone-isometric-game/agent.md) and [manifest](https://agent-buildprint.com/buildprints/standalone-isometric-game/package.json), starting with `BUILDPRINT.md` and `00-goal.md`, then only the applicable `instructions.phaseReadOrder` entry from the [source packet manifest](https://agent-buildprint.com/buildprints/standalone-isometric-game/files/package.json). Direct reading supports alignment/planning without executing a CLI or fabricating state.
