...
Before Phase 01, create the real base project structure plus root `AGENTS.md`, `.buildprint/setup.md`, `architecture.md`, `engineering-standards.md`, `proof-strategy.md`, and `ui-identity.md`. Root `AGENTS.md` must list those files as mandatory reads before code edits. `architecture.md` must define the base project structure and Boundary map. `engineering-standards.md` must include clean code rules, validation and schemas, persistence standards, provider standards, worker/runtime standards, UI standards, and test standards with deterministic exits for blocked proof.
