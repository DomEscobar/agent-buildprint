...
Selected blueprint mode: `product`. Phase style: `outcome_flow`. Implement a source-independent Novel Storyboard Pipeline; do not preserve legacy branding or require original source access.
...
Before Phase 01, satisfy the Foundation scaffold gate: create the selected stack's real base project structure plus root `AGENTS.md`, `.buildprint/setup.md`, `architecture.md`, `engineering-standards.md`, `test-strategy.md`, and `ui-identity.md`. Root `AGENTS.md` must list these as mandatory reads before code edits. `architecture.md` must include Boundary map and dependency rules. `engineering-standards.md` must include clean code rules, validation, persistence, provider, worker/runtime, UI, and Test standards with deterministic blocked-proof exit behavior.
