...
Proof id: proof-tools-skills-mcp-policy
...
Required runtime evidence row must use `phase_id: tools-skills-mcp-policy` and write to `.buildprint/evidence/evidence-ledger.jsonl` after phase-flow artifacts exist. The packaged `05-evidence/evidence-ledger.jsonl` is seed evidence only.
