...
Your job is to produce a phase plan, assemble the right review lenses, dispatch bounded work or explicitly simulate bounded specialist passes when subagents are unavailable, review returns, integrate, verify, and record proof. Do not implement everything first and explain the process afterward.
...
4. Dispatch specialist work when subagents are available, or if subagents are unavailable, simulate each role explicitly and write the simulated return.
