...
2. `UI-IDENTITY.md` because this packet is UI-bearing. It must be produced by an explicit UX/UI persona pass before UI implementation begins and must define a polished deck-workbench identity: canvas-first layout, slide/outline/chat/export interaction density, motion and transition rules, clickable-control rules, responsive behavior, accessibility baseline, component/state matrix, screenshot critique rubric, and forbidden generic/default dashboard patterns. Root `AGENTS.md` must list it as a mandatory read.
3. `.env.example` with provider/runtime/storage/export configuration and no real secrets.
...
7. Verification commands for install/setup, dev, build, test, and browser/API/export smoke review.
