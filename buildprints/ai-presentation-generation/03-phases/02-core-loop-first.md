...
Build the smallest real prompt/document/template-to-outline-to-deck loop before expanding features.
...
Implement deck creation, content ingestion seam, outline generation or blocked provider path, and persisted deck model with slide placeholders derived from actual input.
...
A user input produces/reloads a real deck record or an honest blocker.
