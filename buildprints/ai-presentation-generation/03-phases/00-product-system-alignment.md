...
Implement the running architectural base designed in `02-project-setup.md` for AI Presentation Generation. Do not re-litigate setup and do not write another alignment essay. Turn the setup decisions into a runnable skeleton that future phases can extend without inventing architecture.
...
- Create the app/runtime skeleton for trusted-local Presenton-inspired AI presentation generation workbench.
- Implement ProviderAdapter, ImageProviderAdapter, DocumentIngestion, DeckModel, ExportRuntime, AutomationBoundary, webhook/MCP/desktop seams as adapter stubs or precise seams.
- Show missing provider credentials, parser dependencies, image provider keys, LibreOffice/Chromium/export runtime, webhook receiver, MCP/desktop packaging, and public deployment controls as honest blockers.
...
Build the foundation skeleton only: deck workbench skeleton with provider/export readiness, deck routes, adapter stubs, persistence init, health/config endpoint, and smoke command.
...
A future agent can run one setup/dev command and see the correct domain skeleton. The first surface must express deck model: prompt/document/template input, outline, slides, layouts, assets, chat iteration, exports, and automation tasks, not a generic dashboard.
...
Create adapter interfaces/stubs with live/blocker/error behavior for ProviderAdapter, ImageProviderAdapter, DocumentIngestion, DeckModel, ExportRuntime, AutomationBoundary, webhook/MCP/desktop seams.
