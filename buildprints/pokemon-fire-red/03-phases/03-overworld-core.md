...
Read phase-flow CP-A, `references/asset-policy.md`, `references/world-art-sources.md`, and `references/world-verification.md`. Load Tiled maps, implement grid movement with collision, ledges, and map transitions.
...
- `.buildprint/decisions.md` records the confirmed `safe_cc0_default` + `external_sprite_sheets` choice and packet path `assets/world/`
...
- `public/assets/ow/player-npc.png` and `public/assets/tilesets/kanto-world.png` were copied from the packet's committed `assets/world/runtime/` bundle
- player frame rectangles, texture key, foot point, world tile formula, and integer rendering match `alignment-slice/ALIGNMENT.md` or have stronger replacement evidence
...
- `npm run maps:render-proof -- --maps pallet_town,route_1,viridian_city` renders all three current maps through the production loader and binds PNG hashes to current TMX hashes
- `npm run world:traverse-proof -- --checkpoint CP-A` saves a browser trace proving continuous Pallet → Route 1 → Viridian traversal with debug controls disabled
