# Loop 02 — Engine Shell (Phaser Scenes + Game State)

## How to implement this loop

Read `01-setup.md`, `02-identity.md`, architecture/system-architecture.md. Wire Phaser 3 with scene graph and empty game state store.
...
Foundation shell for the battle-first route: boot → title → typed test-fixture entry seam → registered production `BattleScene` → title. This loop proves architecture, input, scaling, and scene contracts only; it does not claim a playable battle or overworld.

The deterministic fixture entry is test/dev-only and absent from release navigation. It may pass typed initial state to the production BattleScene in Loop 03 but may never mutate outcomes or introduce a separate renderer or reducer.

### Scene graph

1. **BootScene** — load manifest, fonts, UI atlas
2. **TitleScene** — NEW GAME, CONTINUE (disabled if no save), OPTIONS stub
3. **BattleScene** — production scene boundary with typed initial-state/result contracts; implementation and certification belong to Loop 03
4. **OverworldScene** — registered scene boundary only; semantic map loader and Pallet implementation belong to Loop 04
