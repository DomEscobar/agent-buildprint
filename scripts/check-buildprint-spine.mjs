...
for (const slug of slugs) {
  ...
  const validation = fs.readFileSync(path.join(dir, 'VALIDATION_TEMPLATE.md'), 'utf8');
...
console.log(`Buildprint spine check passed: ${slugs.length} package(s), ${required.length} required core files each.`);
