...
for (const slug of slugs) {
  ...
  if (slug === 'buildprint-mapper-os') {
    ...
    const mapperRequired = [
      ...
      ['executable blueprint project setup template', fs.readFileSync(path.join(dir, 'templates/executable-packet/02-project-setup.md'), 'utf8'), /## Setup defaults[\s\S]*## (Product shape|Product \/ capability shape)[\s\S]*## Architecture decisions[\s\S]*## Production readiness contract[\s\S]*## Implementation project setup[\s\S]*## Phase start gate/i],
      ...
      ['executable blueprint overview template', fs.readFileSync(path.join(dir, 'templates/executable-packet/BUILDPRINT.md'), 'utf8'), /Product brief[\s\S]*Required read order[\s\S]*01-questions\.md[\s\S]*02-project-setup\.md[\s\S]*03-phases\/phase-flow\.md[\s\S]*Implementation loop[\s\S]*Repair routing/i],
