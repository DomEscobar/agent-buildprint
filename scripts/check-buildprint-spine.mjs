...
for (const slug of slugs) {
  ...
  if (slug === 'buildprint-mapper-os') {
    ...
    const mapperRequired = [
      ...
      ['executable blueprint project setup template', fs.readFileSync(path.join(dir, 'templates/executable-packet/02-project-setup.md'), 'utf8'), /## Setup defaults[\s\S]*## Product shape[\s\S]*## Architecture decisions[\s\S]*## Production readiness contract[\s\S]*## Implementation project setup[\s\S]*## Phase start gate/i],
