...
function usage() {
  return [
    ...
    '  3. Packet + selected-output checks validate executable packet shape',
...
fs.writeFileSync(path.join(outDir, '03-phases/phase-index.yaml'), 'schema_version: mapper-os/phase-index\nactive_phase: 03-phases/01-document-ingestion-extraction.md\nphases:\n  - phase_id: document-ingestion-extraction\n    file: 03-phases/01-document-ingestion-extraction.md\n    depends_on: []\n    proof_gate: proof-document-ingestion-extraction\n');
  ...
  fs.writeFileSync(path.join(outDir, '05-evidence/evidence-ledger.jsonl'), '{"artifact_id":"dry-taxhacker-seed","type":"source_mapping_seed","status":"proof_required","source":"vas3k/TaxHacker","proves":"dry-run packet mechanics only","phase_id":"document-ingestion-extraction","proof_type":"seed","provider_mode":"none","upgrades_claim":false}\n');
  fs.writeFileSync(path.join(outDir, 'generated/agent-prompt.md'), '# Generated agent prompt\n\nGenerated from: blueprint.yaml\n\nThis file is not source of truth and not authoritative. Follow BUILDPRINT.md, 01-questions.md, 02-project-setup.md, 03-phases/phase-index.yaml, 03-phases/phase-flow.md, 04-evaluation.md, and 05-evidence/evidence-ledger.jsonl.\n');
}
