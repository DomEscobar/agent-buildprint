...
{
  echo "== Overnight remap started =="
  date -Is
  git status --short
  git checkout -B remap-old-buildprints-20260525
...
PROMPT
    ...
    git add "buildprints/$slug" "$LOG_DIR/$slug".*
  ...
  npm run check:publication
  npm run check:spine
  for slug in "${SLUGS[@]}"; do node scripts/check-mapper-selected-output.mjs "buildprints/$slug"; done
  git diff --check
  git status --short
  git log -3 --oneline
  date -Is
  notify "Overnight remap finished: Personal Agent Chat OS, AI Shorts Studio, and Perfect RAG OS were remapped on branch remap-old-buildprints-20260525. Check /root/blueprint/quality/overnight-remap-20260525 for reports."
} 2>&1 | tee "$LOG_DIR/overnight.log"
