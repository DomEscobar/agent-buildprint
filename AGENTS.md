...
- Verify with `agb harness check .` before loop implementation; use `agb harness checkup .` for the stricter setup doctor.
...
- Use `setup-runbook` during `01-setup.md`; it must leave `docs/architecture.md`, `.env.example`, and `.buildprint/setup-receipt.md` or honest blockers.
- Use `frontend-ui-product-design` before building or changing UI.
- Use `subagent-driven-implementation` only for multi-task loops with clean file ownership (independent fan-out).
- Use `verify-and-review` before claiming a loop, checkpoint, or handover is done; finish with independent contract review.
